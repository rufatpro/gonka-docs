...
- 参与者（验证者）通过 PR 提交信息和离线交易文件（`GENTX` 和 `GENPARTICIPANT`）。
...
??? note "创世仪式的关键原则"
    ...
    | **可验证状态** | 记录最终的 `genesis.json` 哈希，允许每个参与者确认他们从完全相同的初始状态开始。 |
    | **共识** | 该过程保证所有初始验证者在网络上线前已审查并接受创世状态。 |

## 先决条件

在参与仪式之前，每个参与者（验证者）必须：

1. Fork [Gonka 仓库](https://github.com/gonka-ai/gonka/) 到你的 GitHub 账户。

2. 选择一个参与者（验证者）名称并创建你的验证者目录：
   ```bash
...
仪式遵循 5 阶段流程，用离线、基于 PR 的工作流程替换 *quickstart.md* 中的链上注册步骤。

!!! info "阶段 1 — 验证者"
    准备密钥和初始服务器设置。  
    打开包含验证者信息的 PR（`节点 ID`、`ML 运营地址`、`共识公钥`）。

!!! tip "阶段 2 — 协调者"
    聚合验证者信息并发布 `genesis.json` 草案供审查。

!!! info "阶段 3 — 验证者"
    从草案生成离线 `GENTX` 和 `GENPARTICIPANT` 文件。  
    打开包含这些文件的 PR。

!!! tip "阶段 4 — 协调者"
    验证并收集交易，修补 `genesis.json`，并设置 `genesis_time`。

!!! info "阶段 5 — 验证者"
    检索最终 `genesis.json`，验证其哈希，并在 `genesis_time` 之前启动节点。
...
!!! note 
    `$KEY_NAME` 和 `$KEYRING_PASSWORD` 在快速开始 `config.env` 中定义。
...
- `MsgSubmitNewParticipant` - 将你的节点注册为网络参与者

`gentx` 命令需要来自前面步骤的以下变量：

| **变量** | **描述** |
|----------|-------------|
| `<cold key name>` | 本地注册表中的账户冷密钥名称（例如，快速开始中的"gonka-account-key"） |
...
#### 3.1. [本地]：创建 `GENTX` 和 `GENPARTICIPANT` 文件
